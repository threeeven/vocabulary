...
export class StudySession {
  ...
  async fetchStudyWordsFromNetwork(dailyGoal) {
    ...
    console.log('📅 时间范围:', todayRange)
    
    // 使用 Promise.all 并行查询
    const [reviewRecords, newWordsCount] = await Promise.all([
      this.getReviewWords(todayRange),
      this.getNewWordsCount(todayRange)
    ])
  ...
  // 获取复习单词
  async getReviewWords(todayRange) {
    const { data, error } = await this.supabase
      .from('study_records')
      .select(`
        id,
      ...
      .gte('next_review_at', todayRange.start)
      .lte('next_review_at', todayRange.end)
  ...
  // 获取今天的时间范围（UTC时间，用于查询next_review_at）
  getTodayTimeRange() {
    const now = new Date()
